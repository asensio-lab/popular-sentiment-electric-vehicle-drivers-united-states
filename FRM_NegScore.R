...
#################### PROCESSING AND VARIABLES MANIPULATION - Agreggating location information 

'Creating Locational data to be named: LocData with specific information to be needed in the proposed models'
LocData <- NALoc_data[,c("Id","Name", "Connectors_site", "Has.DCFC?", "Has.J-1772?","PlugScore")]
'Id, Name, Connectors_site, DCFC, J-1772 -> 5 variables with 20303 obs'
...
final_data_file_2 <- merge(x = final_data_file, y = LocData, all.x = TRUE,by = cbind("Location.Id", "Name", "Connectors_site"), sort = F)
...
new_data_file <- final_data_file_2[, c("Location.Id","Name","Connectors_site", "Id..Review.","User.Id", "name..vehicle."
                                        ,"Created.At..Review.","Networks_site", "Review", "Predicted.Sentiment", "POI..group.", "Day.of.Created.At"
                                        ,"reviewCount","bestPOIGroup", "publicPrivate", "DCFC", "J1772", "PlugScore")]
...
temp_NAReviews <- NAReviews_data[,c("Id..Review.","User.Id","Created.At..Review.", "Location.Id", "Review")]
...
#276650 obs of 6 variables

'Merge into the new_data_file'
new_data_file_plus <- merge(x=temp_NAReviews,y=new_data_file,by= cbind("Id..Review.", "User.Id", "Location.Id", "RYear"), all.x =TRUE, sort = F)
#276650 obs of 22 variables 
...
temp1 <- new_data_file_plus[,c("Id..Review.","Location.Id","RYear", "check_NotWR_plus_WR_SUM")]
...
#36211 obs of 3 variables

'For checkins only with written reviews'
temp2 <- new_data_file_plus[,c("Id..Review.","Location.Id","RYear", "check_WR")]
...
#################### Creating new variables as input for the proposed models
...
new_data_file$Total_Connectors = rowSums(new_data_file[, c(26:34)])

#################### Creating Dummies - Networks
'Total Networks'
new_data_file$NET <- NA 
new_data_file$NET <- as.character(new_data_file$Networks_site)
...
'new_data_file has 127257 obs and 56 variables'

#################### Manipulating variable for "Urban and Non-Urban center"
'Aggregating by Location Id'
subset_UrbInfo <- final_data_file[,c("Id..Review.","User.Id","Location.Id", "urbanRural")]
'4 variables with 127257 obs'
...
#################### Creating Dummies for Predicted Sentiment
...
'new_data_file has 127257 obs of 95 variables'

#################### Count Reviews and Negative Reviews Variable as input for the proposed models
...
#################### Renaming variables for a clean table
...
histogram( ~ Review_Rate | bestPOIGroup , data=new_data_file, 
           layout=c(3,9) , scales= list(y=list(relation="free"),
                                        x=list(relation="free") ) )

histogram( ~ Neg_Score | bestPOIGroup , data=new_data_file, 
           layout=c(3,9) , scales= list(y=list(relation="free"),
                                        x=list(relation="free") ) )
...
####################Review Rate ALL###############################################################
##################################################################################################
#################### Supplementary Table 2 Model FRM (1)
...
#################### Supplementary Table 2 Model FRM (2)
y <- new_data_file$Review_Rate_ALL
...
#################### Supplementary Table 2 Model FRM (3)
y <- new_data_file$Review_Rate_ALL
...
#################### Supplementary Table 2 Model FRM (4)
...
#################### Supplementary Table 2 Model FRM (5)
y <- new_data_file$Neg_Score
...
#################### Supplementary Table 2 Model FRM (6)
y <- new_data_file$Neg_Score
...
################### Supplementary Table 7 Model FRM - Review Rate - Model I
y <- new_data_file$Review_Rate_ALL
xmain7frm1 <- new_data_file[,c("DUrban_Center", "DRural","Dummy_PUBLIC", "Dummy_Government",
...
mainfrm7frm1 <- frm(y , xmain7frm1 , linkfrac = 'logit', var.type =  "cluster", var.cluster = new_data_file$Location.Id)
mainres7frm1 <- frm.pe(mainfrm7frm1)





#################### Supplementary Table 7 Model OLS - Review Rate - Model II
...
#################### Supplementary Table 7 Model FRM Negative Score - Model III
y <- new_data_file$Neg_Score
...
#################### Supplementary Table 7 Model OLS Negative Score Model IV
...
#Calculate mean negativity score using aggregate function
reduced_data <- new_data_file[,c("Location.Id","Predicted.Sentiment","RYear" )]
reduced_data$sentiment <- ifelse(reduced_data$Predicted.Sentiment == "POSITIVE", 0, 1)
agg = aggregate(reduced_data, by = list(reduced_data$Location.Id, reduced_data$RYear),FUN = mean)
mean(agg$sentiment)
