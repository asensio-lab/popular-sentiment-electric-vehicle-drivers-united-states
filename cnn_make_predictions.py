...
# Trains the CNN model and used it to generate predictions on all 140,188 reviews
def main():
    df = pd.read_csv('training_data.csv')

    # Read in file with all reviews
    all_reviews = pd.read_csv('NA Reviews Data.csv', encoding='latin1')

    # Drop rows with no reviews
    all_reviews = all_reviews[pd.notnull(all_reviews['Review'])]
    ...
    # prediction set too
    all_reviews['Review'] = all_reviews['Review'].apply(clean_text)
    ...
    # Reviews to use for training and predicting
    reviews_train = df['Review']
    reviews_predict = all_reviews['Review']

    # Ground truth labels for training
    y_train = df['Sentiment']

    print('Train size: %s' % len(reviews_train))
    print('Predict size: %s' % len(reviews_predict))

    # Convert numpy arrays to lists
    reviews_train = list(reviews_train)
    reviews_predict = list(reviews_predict)
    ...
    x_predict_tokens = tokenizer.texts_to_sequences(reviews_predict)

    # Padd all sequences of tokens to be the same length (length of the longest sequence)
    ...
    x_predict_pad = pad_sequences(x_predict_tokens, maxlen=max_tokens, padding=pad, truncating=pad)
    ...
    print('Predicting sentiment for entire dataset...')
    t0 = time()

    # Make predictions
    y_pred = model.predict(x_predict_pad)
    duration = time() - t0
    print("done in %fs" % (duration))
    y_pred_class = np.round(y_pred, 0)

    # Save predictions to file
    all_reviews['Predicted Sentiment'] = y_pred_class
    reverse_mapping = {0.0: 'NEGATIVE', 1.0: 'POSITIVE'}
    all_reviews['Predicted Sentiment'] = all_reviews['Predicted Sentiment'].map(reverse_mapping)
    all_reviews.to_csv('NA Reviews Data Sentiment.csv')
