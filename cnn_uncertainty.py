...
def main():
    ...
    for i in range(num_iteration):
        ...
        result = pd.DataFrame({'Accuracy':[accuracy],'Precision':[precision], 'Recall': [recall], 'F1 Score': [f1_score]})
    
        result.to_csv('cnn_uncertainty_append.csv', index=True, mode=  'a', header=False)
    ...
    f1_list = 2*recall_list*precision_list/(recall_list+precision_list)
    ...
    plt.figure(1)
    plt.subplot(211)
    plt.plot(range(num_iteration), accuracy_list, 'bo-', )
    
    plt.subplot(212)
    plt.plot(range(num_iteration), f1_list, 'ro-')
    plt.show()

    print('Mean Accuracy: %.2f%%' % accuracy_list.mean())
    print('Min Accuracy: %.2f' % accuracy_list.min())
    print('Max Accuracy: %.2f' % accuracy_list.max())    
    print('Accuracy SD: %.2f' % accuracy_list.std())    


    print('Mean F1 Score: %.2f%%' % f1_list.mean())
    print('Min F1 Score: %.2f' % f1_list.min())
    print('Max F1 Score: %.2f' % f1_list.max())    
    print('F1 Score SD: %.2f' % f1_list.std())  
    
    result_summary = pd.DataFrame({'Accuracy':accuracy_list,'Precision':precision_list, 'Recall': recall_list, 'F1 Score': f1_list})
    
    result_summary.to_csv('cnn_uncertainty_batch.csv', index=True, mode=  'a', header=False)
